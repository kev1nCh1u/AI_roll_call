...
num = ["1105164318","1105105340","1105448787","6644666666"]
att = ["X", "X", "X", "X"]
student_list = {"班級": cl,  
        "學生": name,
        "學號":num,
...
select_df.to_csv(r"" + path + "\\student.csv",index=False,encoding='utf_8_sig')
...
def checkStudent(student):    
    for i in range(0, len(name)):
        if  student == name[i]:
            print(i)
    out_df = select_df[select_df.loc[:,"學生"] == student].index
    select_df.iloc[out_df.values,3] = "V" 
    out_df1 = select_df[select_df.loc[:,"學生"] == student] #印出確認
    print(out_df1)
    select_df.to_csv(r"" + path + "\\student.csv",index=False,encoding='utf_8_sig')

for i in ["邱暐凱","吳定軒","劉建廷","林舒潔"]:
    checkStudent(i)
